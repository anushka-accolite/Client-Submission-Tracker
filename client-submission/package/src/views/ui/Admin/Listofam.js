...
export default function Listofam() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        const filteredDetails = details.filter(item => item.userRole.replace(/\s+/g, '').toLowerCase() === toBeSearched);

        const mappedDetails = filteredDetails.map(item => ({
          Id: item.userId,
          Username: item.userName,
          Email: item.email
        }));
  ...
  return (
    <>
      <h2 id='hd'>List of Account Manager</h2>
      <div className='uppercontainer'>
        <FormControl variant="outlined">
          <Select
            className='filter-select'
